...
### Load Collateral
load(paste0(filepath, "/collateral_columns.RData"))
### Load pefindo model
model_xgb_load = readRDS(file = paste0(filepath, "/model_xgb_24MAY2020_1436.RDS"))
### Load pefindo variable importance
xgb_importance = read.csv(file = paste0(filepath, "/xgb_importance_24MAY2020.csv"))
### Load SLIK model
model_xgb_SLIK = readRDS(file = paste0(filepath, "/Dev_Hit_model_xgb.RDS")) 
### Load SLIK variable importance
xgb_importance_SLIK = read.csv(paste0(filepath,'/xgb_importance_dimred.csv'), stringsAsFactors = FALSE) 
### Load amenity data
amenity <- read.csv(paste0(filepath, "/AMENITY.csv")) 
### Load demographic model
model_xgb_demog = readRDS(file = paste0(filepath, "/model_cust_VERSION_D2_11JUNE.RDS"))
### Load banking model
model_banking = readRDS(file = paste0(filepath, "/model_bank_optimized_8JUNE1800.RDS"))
### Load digital model
model_digital = readRDS(file = paste0(filepath, "/model_digital_optimized_25MAY2020_2248.RDS")) 
### Calibration constants
calibrated_constants <- readRDS(paste0(filepath, "/20200901_all_calibrated_constants.RDS"))
...
pdscore_bureau_pefindo <- function(CONTRACT_PAYMENT_CALENDAR_MASTER,CONTRACT_SUMMARY_DEBTOR,MASTER_DATA,
  ...
  scored_data_bureau <- cbind(Final_data_ori,xgb_bureau)
  
  scored_data_bureau_final <- scored_data_bureau %>%
    select(CRN,xgb_bureau) 
...
#### 6. DEM. FUNGSI MENGHITUNG MODUL DEMOGRAFIK NASABAH ####
...
NTB_PDScore_final <-function(scored_data_bureau_final,scored_data_demog_final){
  ...
  pd_database_to_los <- pd_database[,c("Name","ApplicationNumber","GUID","MarketingCode","CustomerType","IDNumber", "CIF","pd")]
...
ETB_PDScore_final <-function(scored_data_bureau_final,scored_data_demog_final,scored_banking_final,scored_digital_final){
  ...
  pd_database <- scored_data1[,c("ApplicationNumber","GUID","Name","MarketingCode","CustomerType","IDNumber", "CIF", "PD_demogs_calib","PD_banking_calib","PD_bureau_calib", "PD_digital_calib")]
